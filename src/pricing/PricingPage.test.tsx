...
it('renders public API plans without hardcoded fallback pricing', async () => {
  vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ plans: [{ id: 'api-plan', name: 'API Plan', slug: 'api-plan', description: 'From API', price_cents: 7399, annual_price_cents: 6199, status: 'published', max_workspaces: 1, max_servers: 9, monthly_tokens: 100, input_tokens: 10, output_tokens: 5, over_limit: 'block', default_model_id: 'm', fallback_model_id: 'm', allowed_model_ids: ['m'], features: ['API feature'], visibility: 'public', subscribers: 0 }] }), { status: 200, headers: { 'Content-Type': 'application/json' } }))
